...
where[0]=   new   comefrom("请选择省份！","请选择相应的城市！");
...
function   select()   {
    with(document.user_reg.sheng)   {   var   loca2   =   options[selectedIndex].value;   }
    for(i   =   0;i   <   where.length;i   ++)   {
        if   (where[i].loca   ==   loca2)   {
            loca3   =   (where[i].locashi).split("|");
            for(j   =   0;j   <   loca3.length;j++)   {   with(document.user_reg.shi)   {   length   =   loca3.length;
                options[j].text   =   loca3[j];   options[j].value   =   loca3[j];   var
                    loca4=options[selectedIndex].value;}}
            break;
        }}
    document.user_reg.teacherNativePlacereal.value=loca2+loca4;
}
function   init()   {
